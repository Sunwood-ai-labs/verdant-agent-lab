...
test('captures the complete desktop interaction proof set', async ({ page }, testInfo) => {
  ...
  await page.screenshot({ path: artifactPath(testInfo, 'builder-dragged-v6.png') });
  ...
  await page.screenshot({ path: artifactPath(testInfo, 'builder-reference-v6.png') });
  await expect(page.locator('#stage')).toHaveScreenshot('builder-reference-stage-v2.png', {
    animations: 'disabled',
    maxDiffPixels: 80
  });
  ...
  await page.screenshot({ path: artifactPath(testInfo, 'builder-focus-v6.png') });

  const downloadPromise = page.waitForEvent('download');
  await page.locator('#exportButton').click();
  const download = await downloadPromise;
  await download.saveAs(artifactPath(testInfo, 'verdant-layout-export-v4.json'));
...
test('captures the 390px mobile catalog, stage, and inspector flow', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('#inspectorEmpty')).toBeHidden();
  await page.screenshot({ path: artifactPath(testInfo, 'builder-mobile-full-v6.png'), fullPage: true });
});
